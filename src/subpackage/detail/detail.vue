    ...
    <view class="popup-root">
      ...
      <view class="description">
        ...
        <view class="cell" v-for="item in reasonList" :key="item" @tap="reason = item">
          ...
          <radio class="icon" :value="item" :checked="item === reason" />
        </view>
      </view>
      <view class="footer">
...
const getMemberOrderByIdData = async () => {
  const res = await getMemberOrderByIdAPI(query.id || '1714616959061987329')
...
const onOrderPay = async () => {
  ...
  // 关闭当前页，再跳转支付结果页
  uni.redirectTo({ url: `/subpackage/payment/payment?id=${query.id}` })
}
// 是否为开发环境
  ...
  uni.showModal({
    ...
    success: async (success) => {
      if (success.confirm) {
        ...
        uni.redirectTo({ url: '/subpackage/list/list' })
      }
    },
  })
}
